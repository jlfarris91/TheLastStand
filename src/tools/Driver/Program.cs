...
namespace Driver
{
    class Program
    {
        ...
        private static void DoWork()
        {
            ...
            using (var w3oFile = File.OpenRead("D:\\Projects\\WarcraftIII\\TheLastStand\\_build\\maps\\war3map.w3b"))
            using (var w3oFileReader = new BinaryReader(w3oFile))
            {
                var deserializer = new CustomEntityFileBinaryDeserializer(entityLibrary);
                var entityFile = deserializer.Deserialize(w3oFileReader);
                var tag = Tag.FromString("LTlt");
                var entity = entityFile.OriginalEntries.Find(_ => _.BaseId == tag);
            }
